...
respuestas_predefinidas = {
    ...
    "ayuda": "Puedes preguntarme sobre diversos temas. Si no tengo la respuesta, intentaré buscarla en internet.",
    "tu proposito": "Mi propósito es ser un asistente útil, proporcionando información y apoyo.",
    "inteligencia artificial": "La inteligencia artificial (IA) es un campo de la informática que crea sistemas capaces de aprender y resolver problemas.",
    "wikipedia": "Wikipedia es una enciclopedia libre y gratuita en línea.",
    "youtube": "YouTube es una plataforma de videos propiedad de Google."
}
...
# --- BARRA LATERAL ---
# AQUÍ ESTÁ EL MENÚ, SOLO UNA VEZ
with st.sidebar:
    st.header("🤖 IA Libre")
    st.markdown("---")
    ...
    # LE PUSE UN NOMBRE ÚNICO (key) PARA QUE NO DE ERROR
...
if opcion_seleccionada == "Inicio":
    ...
    st.write(f"Soy **{nombre_ia}**, desarrollado en **{raices_ia}** por **{padre_ia}**. Estoy aquí para ayudarte.")
...
# PÁGINA CHAT
    ...
    st.write(f"¡Hola! Soy {nombre_ia}. Puedo buscar información en tiempo real.")
        ...
        if not encontrado and SERPAPI_API_KEY:
            try:
                ...
                if "organic_results" in resultados and resultados["organic_results"]:
                    ...
                    respuesta_ia = "Busqué en la red pero no encontré datos específicos."

            except Exception as e:
                respuesta_ia = f"Error: {str(e)}"

        elif not encontrado and not SERPAPI_API_KEY:
            respuesta_ia = "Necesito mi clave API para buscar."
...
# PÁGINA SOBRE NOSOTROS
    ...
    st.write(f"**{nombre_ia}** fue desarrollado por **{padre_ia}**.")
    st.write("Utiliza tecnología de Inteligencia Artificial y búsqueda en Bing para entregar información actualizada.")
    st.write(f"Orgullosamente hecho en **{raices_ia}**.")
